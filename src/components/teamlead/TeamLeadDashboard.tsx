...
import { AddPasswordModal } from '../admin/AddPasswordModal';
import { Plus, Key, Eye, EyeOff, Copy, Check, Share2, Trash2 } from 'lucide-react';
...
export function TeamLeadDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<'my_passwords' | 'shared_access'>('my_passwords');
  const [showAddPassword, setShowAddPassword] = useState(false);
  const [myPasswords, setMyPasswords] = useState<PasswordEntry[]>([]);
  ...
  const fetchMyPasswords = async () => {
    ...
    if (error) {
      console.error('Error fetching my passwords:', error);
      return;
    }
  ...
  const renderPasswordCard = (entry: PasswordEntry, showDelete = false) => (
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h2 className="text-3xl font-bold text-slate-900">Team Lead Dashboard</h2>
            <p className="text-slate-600 mt-1">Manage your passwords and view shared access</p>
          </div>
          <button
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
          <div className="border-b border-slate-200">
            <div className="flex">
              <button
                onClick={() => setActiveTab('my_passwords')}
                className={`flex items-center gap-2 px-6 py-4 font-medium transition-colors ${
                  activeTab === 'my_passwords'
                    ? 'text-slate-900 border-b-2 border-slate-900'
                    : 'text-slate-600 hover:text-slate-900'
                }`}
              >
                <Key className="w-5 h-5" />
                My Password Entries ({myPasswords.length})
              </button>
              <button
              ...
              >
                ...
                Shared Access ({sharedPasswords.length})
              </button>
            </div>
          </div>
            ...
            {activeTab === 'my_passwords' && (
              <div className="space-y-4">
                {myPasswords.length === 0 ? (
                  ...
                  myPasswords.map(entry => renderPasswordCard(entry, true))
                )}
              </div>
            )}
            ...
            {activeTab === 'shared_access' && (
              <div className="space-y-4">
                {sharedPasswords.length === 0 ? (
                  ...
                  sharedPasswords.map(entry => renderPasswordCard(entry, false))
                )}
              </div>
            )}
          </div>
